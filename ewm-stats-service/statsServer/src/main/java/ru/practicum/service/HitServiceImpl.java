...
public class HitServiceImpl implements HitService {
    ...
    public HitsDto createHit(HitsDto hitsDto) {
        if (!appDictStorage.existsByAppName(hitsDto.getApp())) {
            AppDict newAppDict = CustomMapper.INSTANCE.toAppDict(hitsDto);
        ...
        if (!uriDictStorage.existsByUriName(hitsDto.getUri())) {
            UriDict newUriDict = CustomMapper.INSTANCE.toUriDict(hitsDto);
        ...
        return CustomMapper.INSTANCE.toHitsDto(newHits);
    }
}
